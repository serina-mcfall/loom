> **ARCHIVED — this is history, not current documentation.**
>
> Moved here 2026-08-05. This is the task-by-task execution plan for Loom v1, which
> was built and merged as PR #2 on 2026-08-03. It is kept because the reasoning in it
> is worth reading, and deleted from nowhere because that reasoning is the record of
> how the thing came to exist.
>
> **Do not read it as a description of the code.** It is 2,700 lines and was the
> largest file in the repository — larger than everything it produced — which made it
> a fourth place the snapshot shape was described and therefore a drift surface. The
> code, `README.md` and `docs/superpowers/specs/2026-08-03-loom-design.md` are
> authoritative; several details here are already superseded (the schema is now 2,
> ranking moved out of `build_snapshot`, staleness no longer consults tmux panes).
>
> Audit finding L12.
...
def _natural(s: object) -> tuple:
    """Sort key where embedded numbers compare numerically, so PR #9 precedes PR #10.

    Each chunk is tagged so ints and strs never meet at the same tuple position —
    an untagged mix raises TypeError and would take the whole strip down.
    """
...
def needs_you(repo: dict) -> list[dict]:
    """Every field access is defensive, and no malformed record is ever skipped.

    A missing key yields a visibly wrong placeholder instead of an exception or a
    silent omission. One bad record must not blank the strip: a vanished entry reads
    as "all is well", which is the failure this whole project exists to prevent.
    """
    items: list[dict] = []
    ...
    for t in repo.get("worktrees", []):
        ...
        if agent.get("state") == "waiting":
            items.append({"rank": 1, "kind": "agent_waiting",
                          "subject": t.get("dir") or "<unnamed worktree>",
                          "detail": "agent is blocked on a prompt"})

    for p in repo.get("prs", []):
        num = p.get("number", "?")
        # A draft is exempt from BOTH rank 2 and rank 4. Failing checks on work still in
        # progress are expected, and ranking them is the crying-wolf that makes a strip
        # unreadable. Deliberate, and pinned by a test.
        if p.get("draft"):
            continue
        if p.get("checks") == "failing":
            items.append({"rank": 4, "kind": "pr_failing", "subject": f"PR #{num}",
            ...
            items.append({"rank": 2, "kind": "pr_awaiting_review",
                          "subject": f"PR #{num}", "detail": "no review yet"})

    for c in repo.get("collisions", []):
        branches = c.get("branches") or []
        if len(branches) < 2:
            continue  # one branch is not a collision, whatever upstream sent
        items.append({"rank": 3, "kind": "collision",
                      "subject": c.get("file") or "<unknown file>",
    ...
    for t in repo.get("worktrees", []):
        ...
        if agent.get("state") in {"stopped", "stale"} and total:
            items.append({"rank": 5, "kind": "stopped_dirty",
                          "subject": t.get("dir") or "<unnamed worktree>",
                          "detail": f"{total} uncommitted files left behind"})

    for f in repo.get("flags", []):
        items.append({"rank": 6, "kind": f.get("kind", "flag"),
                      "subject": f.get("subject") or "<unnamed>",
                      "detail": f.get("detail", "")})

    return sorted(items, key=lambda i: (i["rank"], _natural(i["subject"])))
```
...
**Files:**
- Create: `loom/serve.py`, `bin/loom` (shell shim — NOT `loom` at the repo root, which would collide with the `loom/` package)
        ...
        for repo in snap["repos"]:
            if include_gh:
                cached_gh[repo["name"]] = {
                    ...
                    # NOT snap["generated_at"] — build_snapshot does not carry that field,
                    # so reading it raised KeyError on the first tick and killed the refresh
                    # thread silently, because it is a daemon. Pass the timestamp in.
                    "cached_at": now_iso,
                }
