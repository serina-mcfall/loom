...
def repo_roots(all_repos: bool) -> list[str]:
    ...
    if r.ok and r.stdout.strip():
        return [os.path.dirname(r.stdout.strip().rstrip("/").removesuffix("/.git"))
                if r.stdout.strip().endswith("/.git") else os.path.dirname(r.stdout.strip())]
    return [os.getcwd()]


def build_snapshot(all_repos: bool) -> dict:
    runner = SubprocessRunner()
    repos = []
    for root in repo_roots(all_repos):
        snap = collect(runner, root, DEFAULT_STATE_DIR)
...
def _refresh_loop(all_repos: bool) -> None:
    """git and hooks every 2s; gh at most once a minute, cached in between."""
    ...
    while True:
        now = time.monotonic()
        include_gh = (now - last_slow) >= SLOW_SECONDS
        snap = build_snapshot(all_repos)
        for repo in snap["repos"]:
            if include_gh:
                cached_gh[repo["name"]] = {"prs": repo["prs"], "issues": repo["issues"],
                                           "sources": repo["sources"]}
            elif repo["name"] in cached_gh:
                repo.update(cached_gh[repo["name"]])
        if include_gh:
            last_slow = now
        with _lock:
            _snapshot = snap
...
function renderPrs(repo) {
  const box = el("prs");
  box.replaceChildren();
  const broken = repo.sources.find((s) => s.name === "gh" && !s.ok);
  if (broken) {
    // A broken panel and an empty panel must never look the same.
    box.append(text("p", `Pull requests unavailable — gh: ${broken.error}`, "src--bad"));
    return;
  }
